 ...
 * @returns 是否是白名单成员。
 */
export function isSkinAssetName(name: string): name is SkinAssetName {
  return (SKIN_ASSET_NAMES as readonly string[]).includes(name)
}
...
export function defaultAssetsDir(): string {
  ...
  if (typeof dshHome === 'string' && dshHome !== '') return join(dshHome, 'skins')
 ...
 */
export function resolveAssetsDir(configured: unknown): string {
 ...
 */
export function assetFilePath(dir: string, name: SkinAssetName): string | undefined {
 ...
 */
export async function lookupAsset(dir: string, rawName: string): Promise<AssetLookup> {
...
export function openAsset(path: string): NodeJS.ReadableStream {
 ...
 */
export function describeAssetsDir(configured: unknown): { dir: string; isDefault: boolean } {
